<template>
  <div class="row">
    ...
    <div class="col-md-6 col-sm-12 col-xs-12 q-pr-sm">
      <q-card>
        <q-card-section>
          <HeaderComponent icon="bar_chart" title="My Categories Gains" />
        </q-card-section>
      </q-card>
    </div>
    <div class="col-md-6 col-sm-12 col-xs-12 q-pl-sm">
      <q-card>
        <q-card-section>
          <HeaderComponent icon="bar_chart" title="My Categories Costs" />
        </q-card-section>
      </q-card>
    </div>
  </div>
...
export default {
  ...
  methods: {
    updateChart(type) { //Atualiza o chart
      type == "next" ? this.manager.next() : this.manager.previus();
      this.chart.data.labels = this.manager.months;
      this.chart.data.datasets[0].data = this.sumByMonths("costs", this.manager.months);
      this.chart.data.datasets[1].data = this.sumByMonths("gains", this.manager.months);
      this.chart.update();
    },

    createChart() { //Cria o chart
